...
class Config:
    ...
    # Email Credentials
    EMAIL_SENDER = 'faceattentai@gmail.com'
    EMAIL_PASSWORD = 'hauw gukb ohfl hyes'
    EMAIL_RECIPIENT = 'dhyandev027@gmail.com'
    SMTP_SERVER = 'smtp.gmail.com'
    SMTP_PORT = 587

    # UI Colors
    BG_MAIN = '#2c3e50'       # Dark Blue-Grey
    BG_TITLE = '#1abc9c'      # Teal
    BG_FRAME = '#ecf0f1'      # Light Grey
    BG_BOTTOM = '#34495e'     # Darker Blue-Grey
    BTN_TAKE_ATT = '#3498db'  # Blue
    BTN_REFRESH = '#95a5a6'   # Grey
    BTN_CLEAR_LOG = '#7f8c8d' # Dark Grey
    BTN_EMAIL = '#16a085'     # Dark Teal
    BTN_CLEAR_H = '#f39c12'   # Orange
    BTN_TAKE_IMG = '#2ecc71'  # Green
    BTN_TRAIN = '#e67e22'     # Dark Orange
    TXT_WHITE = 'white'
    TXT_DARK = '#2c3e50'

    # Paths
    BASE_PATH = os.path.dirname(os.path.abspath(__file__))
    HAARCASCADE_FILE = os.path.join(BASE_PATH, HAARCASCADE_PATH)
    TRAINING_IMG_DIR = os.path.join(BASE_PATH, "TrainingImage")
    TRAINING_LBL_DIR = os.path.join(BASE_PATH, "TrainingImageLabel")
    TRAINER_FILE = os.path.join(TRAINING_LBL_DIR, "Trainner.yml")
    STUDENT_DETAILS_DIR = os.path.join(BASE_PATH, "StudentDetails")
...
for folder in [Config.TRAINING_IMG_DIR, Config.TRAINING_LBL_DIR, Config.STUDENT_DETAILS_DIR, Config.ATTENDANCE_DIR]:
...
def check_haarcascadefile():
    ...
    if not os.path.isfile(Config.HAARCASCADE_FILE):
        mess.showerror('Error', f'Critical Error: Haar Cascade file not found!\n{Config.HAARCASCADE_FILE}')
        window.destroy()
        return False
    return True
...
def display_attendance_log():
    ...
    attendance_file_path = os.path.join(Config.ATTENDANCE_DIR, f"Attendance_{today_date}.csv")
    ...
    try:
        ...
        with open(attendance_file_path, 'r', newline='', encoding='latin-1') as csvfile:
            reader = csv.reader(csvfile)
            header = next(reader, None)
            if header is None:
                message_label.config(text=f"Attendance file for {today_date} empty.")
                return
            for row in reader:
                if len(row) >= 4:
...
def take_images():
    ...
    try:
        ...
        if not cam.isOpened(): raise IOError(f"Cannot open webcam {Config.CAMERA_INDEX}")
        face_detector = cv2.CascadeClassifier(Config.HAARCASCADE_FILE)
        student_id_int = int(student_id)

        existing_ids = set()
        if os.path.isfile(Config.STUDENT_DETAILS_FILE):
            try: # Try reading details file robustly
                try: df_details = pd.read_csv(Config.STUDENT_DETAILS_FILE, encoding='utf-8')
                except UnicodeDecodeError: df_details = pd.read_csv(Config.STUDENT_DETAILS_FILE, encoding='latin-1', errors='ignore')
        ...
        if student_id_int in existing_ids:
             ...
             try: # Write details file using UTF-8
                  file_exists = os.path.isfile(Config.STUDENT_DETAILS_FILE)
                  with open(Config.STUDENT_DETAILS_FILE, 'a+', newline='', encoding='utf-8') as csvFile:
                       writer = csv.writer(csvFile)
                       if not file_exists or os.path.getsize(Config.STUDENT_DETAILS_FILE) == 0:
        ...
        while img_count < max_images:
            ...
            for (x, y, w, h) in faces:
                ...
                img_path = os.path.join(Config.TRAINING_IMG_DIR, f"{student_name}.{student_id}.{img_count}.jpg")
            ...
            if cv2.waitKey(50) & 0xFF == ord('q'):
                message_label.config(text="Capture cancelled.")
                print("Capture cancelled.")
                img_count = max_images + 1
                break
...
def train_images():
    ...
    if not check_haarcascadefile(): return
    image_paths = [os.path.join(Config.TRAINING_IMG_DIR, f) for f in os.listdir(Config.TRAINING_IMG_DIR) if f.endswith(('.jpg', '.png', '.jpeg'))]
    ...
    faces = []
    ids = []
    message_label.config(text="Starting training...")
    ...
    try:
        ...
        if not os.path.exists(Config.TRAINING_LBL_DIR): os.makedirs(Config.TRAINING_LBL_DIR)
...
def track_images():
    ...
    if not check_haarcascadefile(): return
    if not os.path.isfile(Config.TRAINER_FILE):
        mess.showerror("Tracking Error", f"Trainer missing:\n{Config.TRAINER_FILE}")
        return
    if not os.path.isfile(Config.STUDENT_DETAILS_FILE):
        mess.showerror("Tracking Error", f"Details missing:\n{Config.STUDENT_DETAILS_FILE}")
        return
    ...
    try:
        ...
        face_cascade = cv2.CascadeClassifier(Config.HAARCASCADE_FILE)
        try: # Read details file robustly
            try: df = pd.read_csv(Config.STUDENT_DETAILS_FILE, encoding='utf-8')
            except UnicodeDecodeError: df = pd.read_csv(Config.STUDENT_DETAILS_FILE, encoding='latin-1')
        ...
        if file_exists and os.path.getsize(attendance_file_path) > 0:
            try: # Read existing attendance robustly
                attendance_df = pd.read_csv(attendance_file_path, encoding='latin-1')
        ...
        message_label.config(text="Tracking... Press 'q' to Stop.")
        window.update_idletasks()
        ...
        while active_tracking:
                ...
                if conf < Config.CONFIDENCE_THRESHOLD:
                    try:
                        ...
                        if not student_info.empty:
                            ...
                            conf_percent = round(100 - conf)
                            display_text = f"{student_name} ({conf_percent}%)"
                        ...
                        else: display_text = f"ID {student_id_str} (Not Registered)"

                    except Exception as e: print(f"Error processing ID {student_id_str}: {e}"); display_text = "Error"
            ...
            if cv2.waitKey(10) & 0xFF == ord('q'):
                ...
                message_label.config(text="Tracking stopped.")
                break
        ...
        if cam and cam.isOpened(): cam.release()
        cv2.destroyAllWindows(); print("Tracking resources released.")

        display_attendance_log()

def generate_and_email_report():
    """Generates and emails the attendance report for today."""
    print("Attempting email report...")
    message_label.config(text="Generating email...")
    window.update_idletasks()
    today_date = datetime.date.today().strftime("%d-%m-%Y")
    attendance_file_path = os.path.join(Config.ATTENDANCE_DIR, f"Attendance_{today_date}.csv")
    ...
    try: # Construct and send email
        ...
        msg['Subject'] = f"Attendance Report - {today_date}"
        ...
        try:
            with open(attendance_file_path, "rb") as attachment:
            ...
            mess.showerror("Email Error", f"Attach failed: {attach_err}")
            message_label.config(text="Email fail: Attach error.")
            return

        print(f"Connecting to SMTP: {Config.SMTP_SERVER}:{Config.SMTP_PORT}")
        ...
        server.quit()
        print("Email sent.")
        mess.showinfo("Email Sent", f"Report sent to {Config.EMAIL_RECIPIENT}.")
        message_label.config(text="Email sent.")
    except smtplib.SMTPAuthenticationError:
        mess.showerror("Email Error", "Authentication Failed. Check email/password (or App Password).")
        message_label.config(text="Email fail: Auth error.")
...
window.configure(background=Config.BG_MAIN)
...
# Header
title_frame = tk.Frame(window, bg=Config.BG_TITLE)
title_frame.pack(fill='x')

tk.Label(title_frame, text="Face Recognition Based Attendance System", font=title_font, fg=Config.TXT_WHITE, bg=Config.BG_TITLE, pady=10).pack()

datetime_frame = tk.Frame(title_frame, bg=Config.BG_TITLE)
datetime_frame.pack()

date_label = tk.Label(datetime_frame, text="--DATE--", font=label_font, fg=Config.TXT_WHITE, bg=Config.BG_TITLE)
date_label.pack(side=tk.LEFT, padx=10)

time_label = tk.Label(datetime_frame, text="--TIME--", font=label_font, fg=Config.TXT_WHITE, bg=Config.BG_TITLE)
time_label.pack(side=tk.LEFT, padx=10)

# Content
content_frame = tk.Frame(window, bg=Config.BG_MAIN)
content_frame.pack(fill='both', expand=True, padx=10, pady=10)

# Left Frame (Attendance Log)
left_frame = tk.LabelFrame(content_frame, text="Attendance Log", font=label_font, bd=2, relief='groove', bg=Config.BG_FRAME, fg=Config.TXT_DARK)
...
button_frame_left = tk.Frame(left_frame, bg=Config.BG_FRAME)
button_frame_left.pack(fill='x', pady=5, padx=5)

tk.Button(button_frame_left, text="Take Attendance", command=track_images, font=button_font, bg=Config.BTN_TAKE_ATT, fg=Config.TXT_WHITE, height=2).grid(row=0, column=0, padx=5, pady=2, sticky='ew')
tk.Button(button_frame_left, text="Refresh Log", command=display_attendance_log, font=button_font, bg=Config.BTN_REFRESH, fg=Config.TXT_WHITE, height=2).grid(row=0, column=1, padx=5, pady=2, sticky='ew')
tk.Button(button_frame_left, text="Clear Log Display", command=clear_treeview, font=button_font, bg=Config.BTN_CLEAR_LOG, fg=Config.TXT_WHITE, height=2).grid(row=1, column=0, padx=5, pady=2, sticky='ew')
tk.Button(button_frame_left, text="Email Today's Report", command=generate_and_email_report, font=button_font, bg=Config.BTN_EMAIL, fg=Config.TXT_WHITE, height=2).grid(row=1, column=1, padx=5, pady=2, sticky='ew')

button_frame_left.grid_columnconfigure((0,1), weight=1)

# Right Frame (Registrations)
right_frame = tk.LabelFrame(content_frame, text="New Registrations", font=label_font, bd=2, relief='groove', bg=Config.BG_FRAME, fg=Config.TXT_DARK)
right_frame.pack(side=tk.RIGHT, fill='y', padx=10, pady=10)

tk.Label(right_frame, text="Student ID:", font=label_font, bg=Config.BG_FRAME).grid(row=0, column=0, padx=10, pady=10, sticky='w')
txt_id = tk.Entry(right_frame, font=label_font, width=15)
txt_id.grid(row=0, column=1, padx=10, pady=10)

tk.Label(right_frame, text="Student Name:", font=label_font, bg=Config.BG_FRAME).grid(row=1, column=0, padx=10, pady=10, sticky='w')
txt_name = tk.Entry(right_frame, font=label_font, width=15)
txt_name.grid(row=1, column=1, padx=10, pady=10)

tk.Button(right_frame, text="Clear Fields", command=clear_id_name, font=button_font, bg=Config.BTN_CLEAR_H, fg=Config.TXT_WHITE).grid(row=2, column=0, columnspan=2, padx=10, pady=10, sticky='ew')

ttk.Separator(right_frame, orient='horizontal').grid(row=3, column=0, columnspan=2, sticky='ew', padx=10, pady=15)

tk.Label(right_frame, text="Step 1: Enter Details & Take Images", font=label_font, bg=Config.BG_FRAME).grid(row=4, column=0, columnspan=2, padx=10, pady=5)
tk.Button(right_frame, text="Take Images", command=take_images, font=button_font, bg=Config.BTN_TAKE_IMG, fg=Config.TXT_WHITE, height=2).grid(row=5, column=0, columnspan=2, padx=10, pady=10, sticky='ew')

tk.Label(right_frame, text="Step 2: Train Recognizer", font=label_font, bg=Config.BG_FRAME).grid(row=6, column=0, columnspan=2, padx=10, pady=5)
tk.Button(right_frame, text="Train Images", command=train_images, font=button_font, bg=Config.BTN_TRAIN, fg=Config.TXT_WHITE, height=2).grid(row=7, column=0, columnspan=2, padx=10, pady=10, sticky='ew')

right_frame.grid_columnconfigure((0,1), weight=1)

bottom_frame = tk.Frame(window, bg=Config.BG_BOTTOM, height=30)
bottom_frame.pack(side=tk.BOTTOM, fill='x')
message_label = tk.Label(bottom_frame, text="Initializing...", font=message_font, fg=Config.TXT_WHITE, bg=Config.BG_BOTTOM)
...
if check_haarcascadefile():
    ...
    message_label.config(text="Critical Error: Haar Cascade missing.")
